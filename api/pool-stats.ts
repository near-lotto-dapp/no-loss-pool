...
export default async function handler(req: any, res: any) {
    ...
    try {
        ...
        const [jomoRes, linearRes] = await Promise.all([
            fetch(process.env.VITE_NEAR_URL!, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(jomoBody) }),
            fetch(process.env.VITE_NEAR_URL!, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(linearBody) })
        ]);
